-- ========================================================
-- Supplier 表新增 code/shortName 字段的数据回填脚本
-- ========================================================
-- 背景：
--   schema.prisma 为 Supplier 新增了 code（NOT NULL UNIQUE）和
--   shortName（NULLABLE）字段。对已有数据的库直接执行
--   `npx prisma db push` 会因「已有行 code 为 NULL 无法建唯一
--   索引」而失败。
--
-- 用法（在项目根目录执行）：
--   sqlite3 prisma/dev.db < prisma/scripts/backfill-supplier-code.sql
--   npx prisma db push
--   npx prisma generate
--
-- 说明：
...
--   2. code 生成规则：G + 三位序号（按 id 升序），与 seed.ts 中
--      的 G001-G003 保持一致。已有 code 的行不会被覆盖。
--   3. shortName 沿用 name 前 4 字作为兜底；如已有值则不覆盖。
-- ========================================================

-- 1. 新增列（若已存在会报错，可忽略）
ALTER TABLE Supplier ADD COLUMN code TEXT;
ALTER TABLE Supplier ADD COLUMN shortName TEXT;

-- 2. 回填 code：仅对 code 为空或 NULL 的行按 id 顺序生成
UPDATE Supplier
SET code = 'G' || printf('%03d', id)
...
-- 5. 验证结果
SELECT id, code, name, shortName, phone, contact FROM Supplier ORDER BY id;
