...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const code = String(row["客户编码"] || "").trim();
      const name = String(row["客户名称"] || "").trim();
      const shortName = String(row["客户简称"] || "").trim() || null;
