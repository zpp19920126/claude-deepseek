...
export async function POST(request: NextRequest) {
  try {
    ...
    const codes = rows
      .map((r) => String(r["分类编码"] || "").trim())
      .filter(Boolean);

    const existingCodes = await prisma.category.findMany({
      where: { code: { in: codes } },
      select: { code: true },
    });
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const code = String(row["分类编码"] || "").trim();
      const name = String(row["分类名称"] || "").trim();
      const shortName = String(row["分类简称"] || "").trim() || null;
      const sortOrderRaw = row["排序"];
      const sortOrder =
            ...
            const msg =
              ...
              err.code === "P2002"
                ? (() => {
                    ...
                    return `${field}"${target === "name" ? item.name : item.code}"已存在`;
                  })()
