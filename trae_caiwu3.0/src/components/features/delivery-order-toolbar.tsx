...
export function DeliveryOrderToolbar() {
  ...
  async function handleExport() {
    try {
      const res = await fetch("/api/delivery-orders/export");
      if (!res.ok) throw new Error("导出失败");
      const blob = await res.blob();
      const disposition = res.headers.get("Content-Disposition") || "";
      let fileName = "配送单列表.xlsx";
  ...
  async function handleDownloadTemplate() {
    try {
      const res = await fetch("/api/delivery-orders/template");
      if (!res.ok) throw new Error("下载失败");
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "配送单导入模板.xlsx";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
  ...
  function handlePrint() {
    ...
    window.open(`/delivery-orders/print?${params.toString()}`, "_blank");
  }
